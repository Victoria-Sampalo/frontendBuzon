...
const GestionUsuarios = () => {
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      ...
      if (esTokenValid.error) {
        ...
        const numeroRegistros = await getCountUsersFilters(token);

        setCantidad(numeroRegistros.total_filas);
        console.log("filas " + numeroRegistros.total_filas);
        //cantidad/limit
        const usuarios = await getAllUsersLimitFilters(token, limit, offset);
  ...
  const handleEditClick = (user) => {
    setEditingUser({ ...user, password: "" }); // Clear password field for editing
  };
  ...
  const handleSaveClick = async (userId) => {
    ...
    const token = obtenerToken();
    const usuarioEditado = { ...editingUser };
    // Remove password from the payload if it's empty
    //Esto asegura que no se muestre ni se envíe la contraseña actual inadvertidamente.
    if (!usuarioEditado.password) {
      delete usuarioEditado.password;
    }

    const updatedUser = await updateUser(token, usuarioEditado);
    console.log("Editando..........");
    console.log(usuarioEditado);
    //Cambia el usuario cambiado
  ...
  return (
    <div className={`contendor-tabla ${style.gestionusuarioscontendor}`}>
      ...
      {users && users.length === 0 && <p>TODAVÍA NO HAY USURIOS REGISTRADOS</p>}

      {users && users.length > 0 && (
        <div className="table-responsive">
          <table className={`table table-striped table-hover table-responsive ${"tabla"}`}>
          <thead>
            <tr>
              <th scope="col">ID</th>
      ...
      {users && users.length > 0 && (
        <div className="pagination">
          ...
          {offset > 0 && (
            <button onClick={() => cambiarPagina("-")} className="btnsinestilo">
              <GrFormPrevious />
            </button>
          )}
